Zinc.App.module "Chat", (Chat, App) ->
  ...
  @addInitializer =>
    ...
    App.vent.on "user_join", (user) =>
      @chat_view.$el.find(".chat-list").append JST["chat/joined"](user)
    App.vent.on "user_leave", (user) =>
      @chat_view.$el.find(".chat-list").append JST["chat/left"](user)

    App.vent.on "user_message", (data) =>
      @chat_view.$el.find(".chat-list").append JST["chat/message"](data)
